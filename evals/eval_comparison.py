...
# Faithfulness: is every claim in the response backed by the matched products
# actually given to the LLM, rather than added by the LLM.
faithfulness_scorer = Faithfulness(llm=evaluator_llm)

# Relevancy: does the response actually address the comparison asked for,
# independent of correctness (a faithful but off-topic answer should still
# score low here).
...
async def evaluate_single_comparison_case(agent, test_case):
    ...
    ProductComparisonAgent.matched_products/product_context -- the same
    matching step compare_products' own LLM call used -- rather than being
    re-derived independently, so faithfulness is scored against what the LLM
    actually saw.
    """
    query = UserQuery(query=test_case["user_input"])
    matched = agent.matched_products(query.query.lower())
    retrieved_contexts = agent.product_context(matched)

    response_text = agent.compare_products(query)["response"]
...
async def main(eval_dataset_path: str):
    """Load dataset, evaluate ProductComparisonAgent's responses, and report per-metric averages."""
    results = []
    ...
    for test_case in dataset:
        result = await evaluate_single_comparison_case(agent, test_case)
...
if __name__ == "__main__":
    import asyncio
    asyncio.run(main("evals/data/comparison_evals.json"))
