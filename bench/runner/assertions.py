"""Run validity assertions (spec §6 step 7; controller ruling P7 on Task 7).
...
def check(cfg, summary: dict, spec_on: bool, thresholds: Thresholds) -> tuple[bool, str | None]:
    ...
    ruling: completed count, error rate, host-memory drift, then (when
    ...
    if completed != cfg.num_prompts:
        return False, f"completed {completed} of {cfg.num_prompts}"
    ...
    host_drift = summary.get("host_share_drift_mb")
    if host_drift is not None and host_drift > thresholds.max_host_drift_mb:
        return False, f"host_share_drift_mb {host_drift} exceeds max_host_drift_mb {thresholds.max_host_drift_mb}"
