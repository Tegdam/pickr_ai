...
- [x] **Add frontend delete button to the sessions sidebar.** ~~The sessions
  sidebar has no delete affordance yet.~~ **Done (2026-09-03):** each
  session row now has a hover-revealed "×" button, wired to the existing
  `DELETE /api/sessions/{conversation_id}` route. Deleting the active
  session starts a fresh one automatically. Verified locally against real
  RDS-backed sessions (created and deleted disposable test sessions rather
  than touching real conversation history) — hover, click-to-delete, and
  delete-while-active all confirmed working, no console errors. Also fixed
  a pre-existing CSS specificity bug found in the same block: hovering a
  session row showed the global `button:hover` accent red instead of the
  intended subtle gray.
